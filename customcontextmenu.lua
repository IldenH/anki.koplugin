...
local function make_button(text, width, cb, enabled_func)
	enabled_func = enabled_func or function()
		return true
	end
	return Button:new({
		text = text,
...
function CustomContextMenu:init()
	...
	local can_prepend = function()
		return self.note.has_prepended_content
	end
	local can_append = function()
		return self.note.has_appended_content
	end
	...
	local remove_prev_sentence = make_button("⏩", btn_width, function()
		prev_s_inc(-1)
	end, can_prepend)
	local remove_prev_char = make_button("1-", btn_width, function()
		prev_c_inc(-1)
	end, can_prepend)
	local append_prev_char = make_button("+1", btn_width, function()
		prev_c_inc(1)
	end)
	local append_prev_sentence = make_button("⏪", btn_width, function()
		prev_s_inc(1)
	end)
	...
	local remove_next_sentence = make_button("⏪", btn_width, function()
		next_s_inc(-1)
	end, can_append)
	local remove_next_char = make_button("-1", btn_width, function()
		next_c_inc(-1)
	end, can_append)
	...
	self.confirm_row = HorizontalGroup:new({
		align = "center",
		make_button("Cancel", btn_width * 2, function()
			self:onClose()
		end),
