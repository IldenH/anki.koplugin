            ...
            { {
                ...
                callback = function()
                    self:set_profile(function()
                        return self:show_custom_context_widget()
                    end)
                end,
            } },
            { {
        ...
        buttons = {
            {
                ...
                },
                {
                    ...
                    callback = function()
                        ...
                        if #new_url == 0 then
                            return UIManager:show(InfoMessage:new({ text = "Empty URL", timeout = 4 }))
                        end
...
function AnkiWidget:buildSettings()
    ...
    local profile_names = {}
    for pname, _ in pairs(Configuration.profiles) do
        table.insert(profile_names, {
            text = pname,
        ...
        menu_item.hold_callback = function()
            UIManager:show(ConfirmBox:new({
                ...
                ok_callback = function()
                    ...
                    if self.ui.menu.onTapCloseMenu then
                        self.ui.menu:onTapCloseMenu()
                    elseif self.ui.menu.onCloseFileManagerMenu then
    ...
    table.insert(profiles, #profiles + 1, {
        text = "Clone profile from ...",
        enabled_func = function()
            return has_profiles
        end,
        sub_item_table = profile_names,
    })
    ...
    return {
        ...
        {
            text = "anki-connect settings",
            keep_menu_open = true,
            callback = function()
                self:show_connection_widget()
            end,
        },
        {
            text = ("Sync (%d) offline note(s)"):format(#AnkiConnect.local_notes),
            enabled_func = function()
                return #AnkiConnect.local_notes > 0
            end,
            callback = function()
                self:check_conn(function()
                    AnkiConnect:sync_offline_notes()
                end)
            end,
        },
    }
...
function AnkiWidget:extend_doc_settings(filepath, document_properties)
    ...
    local metadata = {
        ...
        pages = function()
            return document_properties.pages or self.ui.doc_settings:readSetting("doc_pages")
        end,
    }
    local metadata_mt = {
        __index = function(t, k)
            return rawget(t, k) or "N/A"
        end,
    }
