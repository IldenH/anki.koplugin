...
function AnkiWidget:show_profiles_widget(opts)
	...
	if #buttons == 0 then
		local msg =
			[[Failed to load profiles, there are none available, create a profile first. See the README on GitHub for more details.]]
...
function AnkiWidget:show_config_widget()
	...
	self.config_widget = ButtonDialog:new({
		buttons = {
			{
				{
					text = ("Sync (%d) offline note(s)"):format(note_count),
					id = "sync",
					enabled = note_count > 0,
					callback = function()
						self.anki_connect:sync_offline_notes()
					end,
				},
			},
...
function AnkiWidget:set_profile(callback)
	...
	if user_profile and Configuration.profiles[user_profile] then
		...
		if not ok then
			return UIManager:show(
				InfoMessage:new({ text = ("Could not load profile %s: %s"):format(user_profile, err), timeout = 4 })
			)
		end
		return callback()
	end
	...
	if user_profile then
		info_text = ("Document was associated with the non-existing profile '%s'.\nPlease pick a different profile to link with this document."):format(
			user_profile
		)
	end
