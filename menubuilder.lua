...
for i, x in ipairs(menu_entries) do
	menu_entries[x.id] = i
end
...
function MenuConfigOpt:new(o)
	...
	for k, v in pairs(o.user_conf) do
		new_[k] = v
	end
	for k, v in pairs(o.menu_entry) do
		new_[k] = v
	end
...
function MenuConfigOpt:build_single_dialog()
	...
	local input_dialog =
		build_single_dialog(self.name, self:get_value_nodefault(), self.name, self.description, callback)
...
function MenuConfigOpt:build_list_dialog()
	...
	local input_dialog = build_single_dialog(
		self.name,
		table.concat(self:get_value_nodefault() or {}, ","),
		self.name,
		description,
		callback
	)
	...
	for _, list_item in ipairs(self:default_values()) do
		table.insert(menu_items, {
			...
			checked_func = function()
				return List:new(self:get_value_nodefault() or {}):contains(list_item)
			end,
...
function MenuConfigOpt:build_map_dialog()
	...
	for _, entry_key in ipairs(values) do
		...
		local edit_menu = {
			...
			callback = function()
				return update_map_entry(entry_key)
			end,
		}
		local menu_item = {
			text = entry_key,
...
function MenuBuilder:build()
	...
	for name, p in pairs(config.profiles) do
		...
		local sub_item_table = {}
		local grouping_func = function(x)
			return x.group[2]
		end
		...
		table.sort(sub_item_table, function(a, b)
			return group_order[a.text] < group_order[b.text]
		end)
...
function MenuBuilder:convert_opt(opt)
	...
	if opt.conf_type == "text" then
		sub_item_entry["callback"] = function()
			return opt:build_single_dialog()
		end
	elseif opt.conf_type == "table" then
		sub_item_entry["callback"] = function()
			return opt:build_multi_dialog()
		end
	elseif opt.conf_type == "bool" then
		sub_item_entry["checked_func"] = function()
			return opt:get_value_nodefault() == true
		end
		sub_item_entry["callback"] = function()
			return opt:update_value(not opt:get_value_nodefault())
		end
	elseif opt.conf_type == "list" then
		sub_item_entry["callback"] = function()
			return opt:build_list_dialog()
		end
