...
function PitchAccent:get_pitch_accents(dict_result)
	...
	local function _convert(downstep)
		...
		for idx, mora in ipairs(get_morae()) do
			...
			logger.dbg(
				"EXT: PitchAccent#get_pitch_accent(): determined marking for mora: ",
				idx,
				table.concat(mora),
				marking
			)
...
function PitchAccent:run(note)
	if not self.popup_dict.is_extended then
		self.popup_dict.results =
			require("langsupport/ja/dictwrapper").extend_dictionaries(self.popup_dict.results, self.conf)
